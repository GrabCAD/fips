...
- **01-Nov-2017**:  The MODit3D/fips fork adds FIPS_ROOT environment variable for moving the fips output files (fips-build, fips-deploy, fetched dependencies,and fips itself when cloned by the fips bootstrap script) to an out of tree path.  If FIPS_ROOT is not defined, the behavior is the current default.
